import * as THREE from 'three';
  ...
  computeDistance(){
    ...
    const distArray = []
    for (let index = 3; index < this._points.length; index+=3) {
      v1.set(this._points[index-3], this._points[index-2], this._points[index-1]);
      v2.set(this._points[index], this._points[index+1], this._points[index+2]);
      d += v1.distanceTo(v2);
      this._distances.push(d); 
      distArray.push(d,d);
    } 
    this.setAttribute('distance', new THREE.BufferAttribute(new Float32Array(distArray), 1));    
  }
...
//  ${THREE.ShaderChunk.fog_pars_vertex}
  ...
  THREE.ShaderChunk['meshline_frag'] = 
     ...
     /*glsl*/`${THREE.ShaderChunk.logdepthbuf_pars_fragment}
    ...
    uniform float useDistance;
    uniform float dashArray;// 虚线的最大长度
    uniform float dashOffset;
    uniform float dashRatio;// 虚的占比
    ...
    void main() {
     ...
     ${THREE.ShaderChunk.logdepthbuf_fragment}
        ...
        float  dist = vCounters;
        if( useDistance > 0. ) {
          dist = vDistance;
        }

        if( useDash >0. && dist > solidEnd ) {
          c.a *= ceil(mod(dist + dashOffset, dashArray) - (dashArray * dashRatio));
        }
    ...
    constructor(parameters)
    {
      super({
        uniforms: Object.assign({}, THREE.UniformsLib.fog, {
          ...
          dashRatio: { value: 0.5 },
          useDash: { value: 0 },
          visibility: { value: 1 },// 不对，这个是直接干掉了，我的虚实线还是得自定义啊
          alphaTest: { value: 0 },
          solidEnd: { value: 0 },// 实线结束的位置  ，如果使用虚线才有用
          useDistance: { value: 0 },
      ...
      Object.defineProperties(this, {
        ...
        useDistance: {
          enumerable: true,
          get: function() {
            return this.uniforms.useDistance.value
          },
          set: function(value) {
            this.uniforms.useDistance.value = value
          },
        },
        solidEnd: {
          enumerable: true,
          get: function() {
            return this.uniforms.solidEnd.value
          },
          set: function(value) {
            this.uniforms.solidEnd.value = value
          },
        },
        visibility: {
          enumerable: true,
